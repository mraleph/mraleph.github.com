...
Optimizing compiler looks at the information collected by inline caches and builds _intermediate representation_ (IR) accordingly. IR instructions are usually more specific and low level than normal JS operations. For example if IC for `.x` saw only objects of shape `{x, y}` then optimizer can take an IR instruction that loads property from a fixed offset inside an object and use it to load `.x`. Of course it is unsafe to apply these instruction to arbitrary objects so optimizer prepends a _type guard_ before an unsafe instruction.
